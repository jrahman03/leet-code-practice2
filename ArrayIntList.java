...
// Description: This class contains upToNowTotal, isPairSorted, removeLast, and removeFront
// upToNowTotal returns a new ArrayIntList that contains a running total of the original list. 
// isPairSorted returns whether or not a list of integers is pairwise sorted (true if it is, false otherwise). 
// removeLast takes an integer n as a parameter and that removes the last n values from a list of integers. 
// removeFront removes the first n values from a list of integers.
import java.util.*;
...
// YOUR CODE GOES HERE
    	...
    	public ArrayIntList upToNowTotal() {
    		//Creating a new Array Int List and total variable outside for loop
    		ArrayIntList running_list = new ArrayIntList();
    		int total = 0;
    		// for loop going through the ArrayIntList
    		for(int i = 0; i < size; i++) {
    			
    			//incrementing at each index
    			total += get(i);
    			running_list.add(total);
    		}
    		//returning the total running list
    		return running_list;
    	} 
    	
    	// pre: 0 < size
    	// post: returns true or false whether a pair is sorted
    	public boolean isPairSorted() {
    		//For loop going through the List size 
    	    for(int i = 0; i < size - 1; i+= 2) {
    	    	
    	    	//if statement determeing if the first element in the pair is more than the comparing (next to) element
    	        if(get(i) > get(i+1)) 
    	        	
    	        	//will return false if the first element is greater than the compared element
    	            return false;
    	    }
    	    //will return true if first element is less than the compared element
    	    return true; 
    	} 
    	

    	// pre: int n >= 0 int n =< size
    	// post: removes the last n values from a list of integers. 
    	public void removeLast(int n) {
    		//for loop running until i > n 
    		for(int i = 0; i < n; i++) {
    			if(size > 0) {
    	...
    	//This method is from page 971 of our book, Question 12
    	// pre: int n >= 0 int n =< size
    	// post: removes the first n values from a list of integers.
    	public void removeFront(int n) {
    		//for loop running until i > n
    		for(int i = 0; i < n; i++) {
    			elementData[i] = elementData[n + i];
    	    }
    	    //returning the size of the arrayList minus n
    	    size -= n;
